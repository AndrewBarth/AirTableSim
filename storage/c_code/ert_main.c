...
int getImage(unsigned char imageOut[]);
...
void *baseRateTask(void *arg)
{
  ...
  while (runModel) {
    ...
    if ( (timeTic % 25) == 0) {
       rc = getImage(imageOut);
       memcpy(Top6DOFModel_U.ImageData,imageOut,921600*sizeof(char));
...
//       rc = serialport_write(serial_fd,theMessage);
       ...
       printf("Theta Angle: %7.3f deg\n",Top6DOFModel_Y.filteredSensorData[13]*57.2958);
    }
