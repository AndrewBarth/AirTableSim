...
static void CCONV onAttachHandler(PhidgetHandle ph, void *ctx) {
	...
	*/
	printf("\tSetting DataInterval to 10ms\n");
	prc = PhidgetAccelerometer_setDataInterval((PhidgetAccelerometerHandle)ph, 10);
...
*/
_declspec(dllexport) int Accelerometermain(double* accelx, double* accely, double* accelz, double* timestamp) {
	PhidgetAccelerometerHandle ch = NULL;
	//ALB ChannelInfo channelInfo; //Information from AskForDeviceParameters(). May be removed when hard-coding parameters.
	PhidgetReturnCode prc; //Used to catch error codes from each Phidget function call
	...
	*/
	...
	/*ALB AskForDeviceParameters(&channelInfo, (PhidgetHandle *)&ch);
	...
	*/
	...
	//ALB printf("Setting OnAccelerationChangeHandler...\n");
	//ALB prc = PhidgetAccelerometer_setOnAccelerationChangeHandler(ch, onAccelerationChangeHandler, NULL);
	//ALB CheckError(prc, "Setting OnAccelerationChangeHandler", (PhidgetHandle *)&ch);
	...
	*/
	...
	//ALB printf("Sampling data for 4 seconds...\n");

	//ALB Sleep(4000);
	Sleep(15);

	double accel[3];
	prc = PhidgetAccelerometer_getAcceleration(ch, &accel);
	*accelx = accel[0];
	*accely = accel[1];
	*accelz = accel[2];

	prc = PhidgetAccelerometer_getTimestamp(ch, timestamp);
...
_declspec(dllexport) int AccelerometerLoop(PhidgetAccelerometerHandle ch, double* accelx, double* accely, double* accelz, double* timestamp) {
